...
exports.AddHomeCategory = async (req, res) => {
  ...
  const values = [rahn, ejare, metrajBena, metrajZamin, forooshi, parking, gerage, balcony, wc, taminAbGarm, garmayesh, sarmayesh, sanad, rebuild, direction, room];

  const [home, fields] = await connection.query(homeSQL,values);
...
exports.AddMobileCategory = async (req, res) => {
  ...
  const mobileSQL = `INSERT INTO cat_mobile (brand, status, count_SM, isAsl) VALUES (?, ?, ?, ?)`;

  const values = [brand, status, countSM, Asl];

  const [mobile, fields] = await connection.query(mobileSQL, values);
...
exports.AddCarCategory = async (req, res) => {
  ...
  const { title, content, price, phoneNumber, saveDate, salTolid, karkard, bimeSales, bimeBadane, tarikhBime, brand, shasiStatus, motorStatus, tarikhBimeSales, girboxType, exchange } = req.body;
  ...
  const values = [salTolid, karkard, bimeSales, bimeBadane, tarikhBime, brand, shasiStatus, motorStatus, tarikhBimeSales, girboxType, exchange];

  const [car, fields] = await connection.query(carSQL, values);
...
exports.AddEstekhdamCategory = async (req, res) => {
  ...
  const { title, content, price, phoneNumber, homeWork, hamkariType, pardakhtType, bime, startTime, endTime, thursday, otherTime, experience, sarbazi, disablePerson } = req.body;
  ...
  const values = [homeWork, hamkariType, pardakhtType, bime, startTime, endTime, thursday, otherTime, experience, sarbazi, disablePerson];

  const [estekhdam, fields] = await connection.query(estekhdamSQL, values);
